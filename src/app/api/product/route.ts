...
export async function POST(req: Request) {
  try {
    ...
    if (
      showcaseIdRaw !== null &&
      showcaseIdRaw !== undefined &&
      showcaseIdRaw !== ''
    ) {
      const parsed = Number(showcaseIdRaw);
      showcaseId ? null : parsed;
    }

    // Validate required fields
    if (!shopId) {
      return new Response(
        JSON.stringify({ error: 'Valid shopId is required' }),
        { status: 400 }
      );
    }
    if (!name) {
    ...
    if (isNaN(stock) || stock < 0) {
      return new Response(
        JSON.stringify({ error: 'Valid stock is required' }),
        { status: 400 }
      );
    }
    if (!category || !VALID_CATEGORIES.includes(category as Category)) {
      return new Response(
        JSON.stringify({ error: 'Valid category is required' }),
        { status: 400 }
      );
    }
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      const fileName = `products/${Date.now()}_${i}_${file.name.replace(
        /\s+/g,
        '_'
      )}`;
    ...
    if (uploadedImages.length === 0) {
      return new Response(
        JSON.stringify({ error: 'No valid images were uploaded' }),
        { status: 400 }
      );
    }
