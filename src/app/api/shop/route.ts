...
export async function POST(req: Request) {
	try {
		...
		let userId: number;
		try {
			userId = getFieldNumber(fields.userId);
		} catch {
		...
		if (imageFile) {
			...
			if (file && file.filepath && file.originalFilename && file.mimetype) {
				try {
					...
					const { data, error } = await supabase.storage.from('bucketimage').upload(fileName, fileBuffer, {
					...
					const { data: publicUrl } = supabase.storage.from('bucketimage').getPublicUrl(data.path);
					...
					return Response.json({ error: 'Failed to process uploaded image' }, { status: 500 });
				}
			}
		}
